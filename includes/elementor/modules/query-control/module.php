<?php
namespace WpUlikePro\Includes\Elementor\Modules\QueryControl;

use Elementor\Controls_Manager;
use Elementor\Core\Common\Modules\Ajax\Module as Ajax;
use Elementor\Widget_Base;
use Elementor\Core\Base\Module AS Module_Base;
use WpUlikePro\Includes\Elementor\Modules\QueryControl\Controls\Group_Control_Posts;
use WpUlikePro\Includes\Elementor\Modules\QueryControl\Controls\Group_Control_Query;
use WpUlikePro\Includes\Elementor\Modules\QueryControl\Controls\Group_Control_Related;
use WpUlikePro\Includes\Elementor\Modules\QueryControl\Classes\Elementor_Post_Query;
use WpUlikePro\Includes\Elementor\Modules\QueryControl\Classes\Elementor_Related_Query;
use WpUlikePro\Includes\Elementor\Modules\QueryControl\Controls\Query;
...
class Module extends Module_Base {
	...
	const QUERY_CONTROL_ID = 'wp-ulike-query';
	 ...
	 *                            `wp_ulike_pro/query/[get_value_titles|get_autocomplete]/{user_defined}`.
	 *
	 ...
	 *      user_defined        : will invoke apply_filters using the user_defined value as filter name,
	 *                            `wp_ulike_pro/query/[get_value_titles|get_autocomplete]/display/{user_defined}`
	 *
	...
	public function ajax_posts_filter_autocomplete_deprecated( $data ) {
		...
		switch ( $data['filter_type'] ) {
			...
			case 'author':
				...
				break;
			default:
				$results = apply_filters( 'wp_ulike_pro/query/get_autocomplete/' . $data['filter_type'], [], $data );
		}
	...
	public function ajax_posts_filter_autocomplete( array $data ) {
		...
		switch ( $query_data['object'] ) {
			case self::QUERY_OBJECT_TAX:
				...
				foreach ( $terms as $term ) {
					if ( apply_filters( "wp_ulike_pro/query/get_autocomplete/tax/{$display}", true, $term, $data ) ) {
						$results[] = [
							'id' => $term->{$by_field},
			...
			case self::QUERY_OBJECT_POST:
				...
				foreach ( $query->posts as $post ) {
					if ( apply_filters( "wp_ulike_pro/query/get_autocomplete/custom/{$display}", true, $post, $data ) ) {
			...
			case self::QUERY_OBJECT_AUTHOR:
				...
				foreach ( $user_query->get_results() as $user ) {
					if ( apply_filters( "wp_ulike_pro/query/get_autocomplete/user/{$display}", true, $user, $data ) ) {
						$results[] = [
							'id' => $user->ID,
				...
				break;
			default:
				$results = apply_filters( 'wp_ulike_pro/query/get_autocomplete/' . $query_data['filter_type'], $results, $data );
		}
	...
	public function ajax_posts_control_value_titles_deprecated( $request ) {
		...
		switch ( $request['filter_type'] ) {
			...
			case 'author':
				...
				break;
			default:
				$results = apply_filters( 'wp_ulike_pro/query/get_value_titles/' . $request['filter_type'], [], $request );
		}
	...
	public function ajax_posts_control_value_titles( $request ) {
		...
		switch ( $query_data['object'] ) {
			case self::QUERY_OBJECT_TAX:
				...
				foreach ( $terms as $term ) {
					if ( apply_filters( "wp_ulike_pro/query/get_value_titles/tax/{$display}", true, $term, $request ) ) {
			...
			case self::QUERY_OBJECT_POST:
				...
				foreach ( $query->posts as $post ) {
					if ( apply_filters( "wp_ulike_pro/query/get_value_titles/custom/{$display}", true, $post, $request ) ) {
			...
			case self::QUERY_OBJECT_USER:
				...
				foreach ( $user_query->get_results() as $user ) {
					if ( apply_filters( "wp_ulike_pro/query/get_value_titles/user/{$display}", true, $user, $request ) ) {
						$results[ $user->ID ] = $this->format_user_for_display( $user, $display, $request, 'get_value_titles' );
					}
				}
				break;
			default:
				$results = apply_filters( "wp_ulike_pro/query/get_value_titles/{$query_data['filter_type']}", $results, $request );
		}
	...
	private function get_term_name( $term, $display, $request, $filter_name = 'get_autocomplete' ) {
		...
		switch ( $display ) {
			...
			case 'minimal':
				...
				break;
			default:
				$text = apply_filters( "wp_ulike_pro/query/{$filter_name}/display/{$display}", $term_name, $request );
				break;
		}
		return $text;
	}
	...
	private function format_post_for_display( $post, $display, $data, $filter_name = 'get_autocomplete' ) {
		...
		switch ( $display ) {
			...
			case 'detailed':
				...
				break;
			default:
				$text = apply_filters( "wp_ulike_pro/query/{$filter_name}/display/{$display}", $post->post_title, $post->ID, $data );
				break;
		}
	...
	private function format_user_for_display( $user, $display, $data, $filter_name = 'get_autocomplete' ) {
		switch ( $display ) {
			...
			case 'detailed':
				...
				break;
			default:
				$text = apply_filters( "wp_ulike_pro/query/{$filter_name}/display/{$display}", $user, $data );
				break;
		}
	...
	public function register_ajax_actions( $ajax_manager ) {
		...
		$ajax_manager->register_ajax_action( 'wp_ulike_panel_posts_control_filter_autocomplete', [ $this, 'ajax_posts_filter_autocomplete' ] );
		/**
		 * @deprecated 2.6.0 use new `autocomplete` format
		 */
		$ajax_manager->register_ajax_action( 'query_control_value_titles_deprecated', [ $this, 'ajax_posts_control_value_titles_deprecated' ] );
		$ajax_manager->register_ajax_action( 'wp_ulike_panel_posts_control_filter_autocomplete_deprecated', [ $this, 'ajax_posts_filter_autocomplete_deprecated' ] );
	}
